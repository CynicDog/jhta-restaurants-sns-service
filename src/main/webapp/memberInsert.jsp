...
<%
	...
	try {  
		if (memberType.equals("customer")) {
			...
			customerDao.insertCustomer(customer); // unique constraint on `id` and `phone`
			...
			ownerDao.insertOwner(owner); // unique constraint on `id` and `phone`	
		...
		response.sendRedirect("memberRegFormPage.jsp?message=uniqueConstraint");
	}	
%>